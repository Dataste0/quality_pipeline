...
def adhoc_transform(df, stats, binary_labels=None, excluded_labels=None):
    ...
    # Filter out non-audited jobs
    non_audited_mask = df["job_correct"].isna() | (df["job_correct"].astype(str).str.strip() == "")
    non_audited = int(non_audited_mask.sum())
    stats["skipped_non_audited"] = non_audited
    df = df[~non_audited_mask].copy()

    # Filter out Annotator ID empty
    annotator_empty_mask = df["rater_id"].isna() | (df["rater_id"].astype(str).str.strip() == "")
    annotator_empty = int(annotator_empty_mask.sum())
    stats["skipped_annotator_empty"] = annotator_empty
    df = df[~annotator_empty_mask].copy()
...
def transform(df, metadata):
    stats = {}
    stats["etl_module"] = "ADHOC-a01Hs00001q1a21IAA"
